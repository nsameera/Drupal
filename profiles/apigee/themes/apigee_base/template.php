...
function apigee_base_preprocess_page(&$variables) {
  ...
  if (module_exists('smartdocs') && $variables['node']->type == 'smart_method') {
    $responsive_path = drupal_get_path('theme', 'apigee_responsive');
    drupal_add_css(drupal_get_path('theme', 'bootstrap') . '/css/overrides.css', array('group' => CSS_SYSTEM));
    drupal_add_css($responsive_path . '/css/bootstrap.min.css', array('group' => CSS_SYSTEM));
    drupal_add_js($responsive_path . '/js/bootstrap.min.js', array('group' => CSS_SYSTEM));
  }
}
